...
class Size2D final : public vec<Size2D<T>, T, 2> {
  using Base = vec<Size2D, T, 2>;

public:
  constexpr Size2D() noexcept = default;

  constexpr Size2D(T const width, T const height) noexcept
    : Base{width, height} {
  }
  ...
  constexpr auto set(T const width, T const height) noexcept -> void {
    std::get<0>(this->elements) = width;
    std::get<1>(this->elements) = height;
  }

  constexpr auto set_width(T const width) noexcept -> void {
    std::get<0>(this->elements) = width;
  }

  constexpr auto set_height(T const height) noexcept -> void {
    std::get<1>(this->elements) = height;
  }

  [[nodiscard]]
  constexpr auto to_rectangle() const noexcept -> Rectangle<T> {

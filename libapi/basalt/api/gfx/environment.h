...
public:
  ...
  [[nodiscard]]
  auto background() const noexcept -> Color const&;

  auto set_background(Color const&) noexcept -> void;

  [[nodiscard]]
  auto ambient_light() const noexcept -> Color const&;

  auto set_ambient_light(Color const&) noexcept -> void;

  [[nodiscard]]
  auto directional_lights() const noexcept -> gsl::span<DirectionalLight const>;

  auto add_directional_light(Vector3f32 const& direction, Color const& diffuse,
                             Color const& specular, Color const& ambient)
    -> void;

  auto add_directional_light(DirectionalLight const&) -> void;

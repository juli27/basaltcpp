...
template <typename T>
auto Vec3<T>::length() const noexcept -> T {
...
template <typename T>
auto Vec3<T>::normalize(const Vec3& v) noexcept -> Vec3 {
  return v / v.length();
}
...
template <typename T>
auto operator+(Vec3<T> l, const Vec3<T>& r) noexcept -> Vec3<T> {
  l += r;
  return l;
}

template auto operator+(Vec3<f32>, const Vec3<f32>&) -> Vec3<f32>;
...
template <typename T>
auto operator*(Vec3<T> v, const T s) noexcept -> Vec3<T> {
  v *= s;
  return v;
}

template auto operator*(Vec3<f32>, f32) noexcept -> Vec3<f32>;
...
template auto operator/(Vec3<f32>, f32) -> Vec3<f32>;

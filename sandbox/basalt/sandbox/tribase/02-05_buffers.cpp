...
auto fill_buffers(const span<Vertex> vb, const span<u16> ib) {
  ...
  for (u32 iCube {0}; iCube < NUM_CUBES; iCube++) {
    ...
    for (u16 iVertex {startVertex}; iVertex < startVertex + 8; iVertex++) {
...
auto Buffers::on_draw(const DrawContext& drawContext) -> void {
  ...
  const Vector3f32 up {Vector3f32::up()};
  const auto zAxis {Vector3f32::normalize(lookAt - mCameraPos)};
  ...
  // clang-format off
  const auto view {Matrix4x4f32::translation(-mCameraPos) *
  ...
  cmdList.draw_indexed(0, 0, NUM_CUBES * 8, 0,

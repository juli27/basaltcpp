...
auto message_to_string(const UINT message, const WPARAM wParam,
  ...
  case WM_CREATE: {
    ...
    return fmt::format(
      ...
      "cy={} cx={} y={} x={} style={:#x} lpszName={} lpszClass={} "
      "dwExStyle={:#x}",
      MESSAGE_NAMES[message], createStruct->lpCreateParams,
      fmt::ptr(createStruct->hInstance), fmt::ptr(createStruct->hMenu),
      fmt::ptr(createStruct->hwndParent), createStruct->cy, createStruct->cx,
      createStruct->y, createStruct->x, createStruct->style,
      create_utf8_from_wide(createStruct->lpszName),
      fmt::ptr(createStruct->lpszClass), createStruct->dwExStyle);
  }

  case WM_MOVE:
    return fmt::format("{} x={} y={}", MESSAGE_NAMES[message], LOWORD(lParam),
                       HIWORD(lParam));

  case WM_SIZE:
    return fmt::format("{} type={} width={} height={}", MESSAGE_NAMES[message],
                       size_w_param_to_string(wParam), LOWORD(lParam),
                       HIWORD(lParam));
  ...
  case WM_SETFOCUS:
  case WM_KILLFOCUS:
    return fmt::format("{} otherWnd={}", MESSAGE_NAMES[message],
  ...
  case WM_ENDSESSION:
    return fmt::format("{} ending={} flags={:#x}", MESSAGE_NAMES[message],
                       wParam, lParam);
  ...
  case WM_SETCURSOR:
    return fmt::format(
      "{} wnd={} htResult={} triggerMsg={}", MESSAGE_NAMES[message],
      fmt::ptr(reinterpret_cast<HWND>(wParam)), ht_to_string(LOWORD(lParam)),
      get_message_name_or_hex_id(HIWORD(lParam)));

  case WM_MOUSEACTIVATE:
    return fmt::format(
      "{} tlParentWnd={} htResult={} triggerMsg={}", MESSAGE_NAMES[message],
      fmt::ptr(reinterpret_cast<HWND>(wParam)), ht_to_string(LOWORD(lParam)),
      get_message_name_or_hex_id(HIWORD(lParam)));
  ...
  case WM_NEXTDLGCTL:
    return fmt::format("{} wParam={} isHandle={}", MESSAGE_NAMES[message],
                       wParam, LOWORD(lParam));

  case WM_SPOOLERSTATUS:
    return fmt::format("{} wParam={:#x} numJobs={}", MESSAGE_NAMES[message],
                       wParam, LOWORD(lParam));

  case WM_DRAWITEM:
    return fmt::format("{} id={} DRAWITEMSTRUCT*={}", MESSAGE_NAMES[message],
                       wParam,
                       fmt::ptr(reinterpret_cast<DRAWITEMSTRUCT*>(lParam)));

  case WM_MEASUREITEM:
    return fmt::format("{} id={} MEASUREITEMSTRUCT*={}", MESSAGE_NAMES[message],
                       wParam,
                       fmt::ptr(reinterpret_cast<MEASUREITEMSTRUCT*>(lParam)));

  case WM_DELETEITEM:
    return fmt::format("{} id={} DELETEITEMSTRUCT*={}", MESSAGE_NAMES[message],
                       wParam,
                       fmt::ptr(reinterpret_cast<DELETEITEMSTRUCT*>(lParam)));

  case WM_SETFONT:
    return fmt::format("{} font={} redraw={}", MESSAGE_NAMES[message],
                       fmt::ptr(reinterpret_cast<HFONT>(wParam)), lParam);

  case WM_WINDOWPOSCHANGING:
  case WM_WINDOWPOSCHANGED: {
    auto* windowPos = reinterpret_cast<WINDOWPOS*>(lParam);
    return fmt::format("{} WINDOWPOS: hwndInsertAfter={} hwnd={} x={} y={} "
                       "cx={} cy={} flags={:#x}",
                       MESSAGE_NAMES[message],
                       fmt::ptr(windowPos->hwndInsertAfter),
                       fmt::ptr(windowPos->hwnd), windowPos->x, windowPos->y,
  ...
  return fmt::format("unknown({:#x}) wParam: {} lParam: {}", message, wParam,
                     lParam);
}
